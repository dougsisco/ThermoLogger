...
#define TC_LOW_MAX -250
#define TC_HIGH_MAX 18000
...
void max31856_oneshot_temperature(spi_device_handle_t spi_handle) {
    ...
    // See MAX31856 datasheet p19.
    val &= ~MAX31856_CR0_AUTOCONVERT;
    val |= MAX31856_CR0_1SHOT;
    max31856_write_register(spi_handle, MAX31856_CR0_REG, val);
	// Note the following delay is required: if you shorten it then the temperature reading will be very inaccurate.
    // Datasheet p19 indicates that the conversion takes 143ms to complete.
...
void thermocouple_read_temperature(int chan, max31856_cfg_t *max31856) {
    ...
    // Note there is a 250ms delay in max31856_oneshot_temperature().
    max31856_oneshot_temperature(spi);
    
    // Read the Open-Circuit fault detection bit.
    unsigned char Status = max31856_read_register(spi, MAX31856_SR_REG);
    
    // If a fault was detected..
    if(Status & 1) {
       max31856->thermocouple_c = INVALID_TEMP;
       max31856->thermocouple_f = INVALID_TEMP;
       return;
    }
    
    unsigned char CR0 = max31856_read_register(spi, MAX31856_CR0_REG);
    
    // If no MAX31856 installed..
    if(CR0 != 0x10) {
        max31856->thermocouple_c = INVALID_TEMP;
        max31856->thermocouple_f = INVALID_TEMP;
        return;
    ...
    //float cj = thermocouple_read_coldjunction(max31856);
    
    int Neg = 0;
    
    // Fix sign bit before shift.
    if (tc_temp & 0x800000) {
        tc_temp |= 0xFF000000;
        Neg = 1;
    }
    // Shift-out the bottom 5 bits since they are unused.
    tc_temp >>= 5; 
    // dgs: fix sign bit again!
    if (Neg)
        tc_temp |= 0xFF000000;
    
    // DGS: Cast to int to correct sign.
    float tc_temp_float = (int)(tc_temp);
    
    // Apparently there is no easier way to do this.
    tc_temp_float /= 128.0;
    max31856->thermocouple_c = tc_temp_float;
    
    if (tc_temp_float > TC_HIGH_MAX || tc_temp_float < TC_LOW_MAX) {
        max31856->thermocouple_c = INVALID_TEMP;
        max31856->thermocouple_f = INVALID_TEMP;
        return;
    }
    ...
    //if (chan == 7 && tc_temp_float < 1000)
//    printf("***Status=%u CR0=%u F=%0.2f, C=%0.2f\n", Status, CR0, max31856->thermocouple_f, max31856->thermocouple_c);
    //printf("*************************************************\n");
    return;
}
